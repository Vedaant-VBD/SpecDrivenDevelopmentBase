"""Baseline tests for the Reports API.

Workshop attendees must keep these green throughout the round. New tests for
the CSV export feature go in `tests/test_csv_export.py` (or wherever your spec says).
"""
...
def test_list_reports_omits_internal_fields() -> None:
    """internal_id and owner_email must NEVER appear in a public response."""

    r = client.get("/reports", params={"limit": 200})
    assert r.status_code == 200
    body_text = r.text
    assert "internal_id" not in body_text
    assert "owner_email" not in body_text
    sample = r.json()["items"][0]
    assert "internal_id" not in sample
    assert "owner_email" not in sample


def test_list_reports_rejects_bad_sort_field() -> None:
    r = client.get("/reports", params={"sort": "owner_email"})
